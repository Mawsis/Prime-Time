<x-layout>
    ...
    <div class="flex justify-center items-center bg-black h-screen text-white">
        <div class="absolute m-auto top-0 bottom-0 right-0 left-0 bg-gray border border-gold w-2/5 h-3/4">
            <form action="/register" method="post" class="flex flex-col justify-center items-center p-8 space-y-5">
                <h1 class="text-3xl text-gold">Register</h1>
                @csrf
                <div class="mb-6 w-full">
                    <label for="name" class="block mb—2 uppercase font—bold text-gold">name</label>
                    <input placeholder="Name" class="border border-gold p-2 w-full text-black" type="text" name="name" id="" value="{{ old('name') }}">
                    @error('name')
                ...
                <div class="mb-6 w-full">
                    ...
                    <input placeholder="Email" class="border border-gold p-2 w-full text-black" type="email" name="email" id="" value="{{ old('email') }}">
                    @error('email')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-6 w-full">
                    ...
                    <input placeholder="Password" class="border border-gold p-2 w-full text-black" type="password" name="password" id="" value="{{ old('password') }}">
                    @error('password')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-6 w-full">
                    ...
                    <input placeholder="Confirm Password" class="border border-gold p-2 w-full text-black" type="password" name="password_confirmation" id="" value="{{ old('password_confirmation') }}">
